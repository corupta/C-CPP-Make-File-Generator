...
def checkErrorsAndWarnings():
	...
	for source in sourceFiles:
		...
		if errFiles:
			print 'Error! Below files are included in', source['fileName'] + '.c but was not found:'
...
#	objects = map(lambda source: source['filePath'][:-1] + 'o', sourceFiles)
	...
	makeFile.write('\t\tgcc ' + " ".join(objects) + " -o " + progName + '\n')
#	makeFile.write('\t\trm ' + " ".join(objects) + '\n')
	...
	for source in sourceFiles:
		...
		makeFile.write("\t\tgcc -c $(INCDIR) " + source['filePath'] + "\n\n")
